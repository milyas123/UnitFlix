    ...
    <table className="flex w-full flex-col gap-y-2.5">
      ...
      </thead>
      {
        data && data.length > 0 ?
            ...
            {data?.map((item, index) => (
                <tr key={index} className="flex items-center divide-x divide-platinum bg-white py-2 text-[14px]">
                  ...
                  {type === "properties" ? (
                      ...
                      <td className="w-[19%] px-4 flex flex-wrap gap-2">
                        <Tag
                            type={item?.approvalStatus === 0 ? "pending" : (item?.approvalStatus === 1 ? "accepted" : 'rejected')}/>
                        <Tag type={item?.purpose === 0 ? "sale" : "rental"}/>
                      </td>
                  )}
            ...
            <div className='flex items-center justify-center text-smokeyGrey'>No Property Found</div>
      }
    </table>
  );
};
