...
{
    public class PropertyRequestValidator : AbstractValidator<PropertyWriteDTO>
    {
        #region Constructor
        ...
        /// Default Constructor
        /// </summary>
        public PropertyRequestValidator(ApplicationDbContext dbContext)
        {
            RuleFor(writeDTO => writeDTO.Category)
                .Equal(PropertyCategory.Property)
                .WithMessage("Can only submit request for a property from this API");
            ...
            RuleFor(writeDTO => writeDTO.UserDetail)
                .NotEmpty()
                .WithMessage("User detials is required")
